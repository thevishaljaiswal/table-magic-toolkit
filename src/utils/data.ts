// Define the DataItem type for use in columns and data generation
...
export const createDataItem = (): DataItem => {
  ...
  return {
    ...
    conversionRate: faker.number.float({ min: 0, max: 1, precision: 0.01 }),
    ...
    churnRate: faker.number.float({ min: 0, max: 1, precision: 0.01 }),
...
export const createDataItems = (count: number): DataItem[] => {
